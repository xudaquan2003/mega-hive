...
#  - HIVE_NODE_TYPE                      the node type, possible values: sequencer, full-node, replica-node
#  - HIVE_SEQUENCER_DISCOVERY_SECRET     secret key of sequencer, from which the sequencer's identity is derived
#  - HIVE_SEQUENCER_PUBLIC_KEY           public key of secret key of sequencer
#
...
if [ "$HIVE_NODE_TYPE" != "" ]; then
    FLAGS="$FLAGS --network-node-type $HIVE_NODE_TYPE"
fi

if [ "$HIVE_SEQUENCER_DISCOVERY_SECRET" != "" ]; then
    echo $HIVE_SEQUENCER_DISCOVERY_SECRET /discovery-secret
    FLAGS="$FLAGS --p2p-secret-key /discovery-secret"
fi

if [ "$HIVE_SEQUENCER_PUBLIC_KEY" != "" ]; then
    FLAGS="$FLAGS --sequencer-public-key $HIVE_SEQUENCER_PUBLIC_KEY"
fi
